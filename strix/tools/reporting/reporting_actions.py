...
def create_bug_report(
    ...
    try:
        ...
        tracer = get_global_tracer()
        if tracer:
            report_id = tracer.add_bug_report(
                title=title,
                content=content,
                severity=severity,
            )
            ...
            return {
                "success": True,
                "message": f"Bug report '{title}' created successfully",
                "report_id": report_id,
                "severity": severity.lower(),
            }
        import logging

        logging.warning("Global tracer not available - bug report not stored")

        return {  # noqa: TRY300
            "success": True,
            "message": f"Bug report '{title}' created successfully (not persisted)",
            "warning": "Report could not be persisted - tracer unavailable",
        }
        ...
        return {"success": False, "message": f"Failed to create bug report: {e!s}"}
